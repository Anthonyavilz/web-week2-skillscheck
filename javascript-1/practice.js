...
function iLove(str, strTwo){
	return str + ' loves ' + strTwo
}
...
let doubled = myNumbers.map(function(arr){
	return arr * 2
})
...
let filtered = myNumbers.filter(function(arr){
	return arr > 100
})
...
let total = myNumbers.reduce(function(total, arr){
	return total + arr
})
...
// let myNumbersIndex = myNumbers.forEach(function(index){
// 	myNumbersIndex = myNumbersIndex + index
// })
...
let forTheLoveOfGeorge = notGeorge.map(function(element){
	return element === 'George' ? 'George' : 'George'
})
